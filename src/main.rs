...
fn update(
    ...
    let writestring = format!(
        "{},{},{},{},{},{},{}",
        max_rods, max_pearls, max_attempts, when_found, attempts, total_exec_time, speed
    );

    let mut file = File::create("dream.txt").expect("Could not open the dream.txt file to write.");
    ...
    println!(
        "Attempts:{}, Rods: {}, Pearls: {}, Speed: {} attempts/sec",
        attempts, max_rods, max_pearls, speed
    );
}
...
fn main() {
    ...
    let contents = fs::read_to_string("dream.txt").expect("Something went wrong reading the file");
    ...
    loop {
        ...
        let rods: Vec<u32> = (0..batch_size)
            .into_par_iter()
            .map_init(Xoshiro256PlusPlus::from_entropy, |rng, _x| {
                (
                    rng.gen::<u128>(),
                    rng.gen::<u128>(),
                    rng.gen::<u64>() & 0x1_FFFF_FFFF_FFFFu64,
                )
            })
        ...
        let mut pearls: Vec<(u32, u32)> = rods
            .clone()
            .into_par_iter()
            .map_init(Xoshiro256PlusPlus::from_entropy, |rng, x| {
                (
                    x,
                    (0..GOLDTRADES)
        ...
        if new_len > 0 {
            let new_record = if new_len == 1 {
                pearls
                    .first()
                    .expect("Expected 1 item in list of length 1.")
            } else {
                ...
                pearls
                    .first()
                    .expect("Expected 1 item in list of length greater than 1.")
            };
